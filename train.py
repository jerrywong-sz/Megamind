...
def train_one_epoch(
    ...
    optimizer,
    device,
    config,  # Pass config in so we can read lambda_weight
    scaler=None,
    amp_enabled=False,
):
    ...
    train_mode = config.get("train_mode", "clean")
    ...
    for images, labels, *_ in train_loader:
        ...
        with torch.autocast(
            ...
            if train_mode == "consistency":
                # Unpack the two views generated by your dataset
                clean_images = images[:, 0, :, :, :]
                damaged_images = images[:, 1, :, :, :]
                
                logits_clean = model(clean_images)
                logits_damaged = model(damaged_images)
                
                # Pull lam from the config!
                lambda_weight = config.get("lambda_weight", 1.0)
                loss, _, _ = robustness_loss(logits_clean, logits_damaged, labels, lam=lambda_weight)
                
                # Use clean logits to calculate training accuracy
                logits = logits_clean
            else:
                logits = model(images)
